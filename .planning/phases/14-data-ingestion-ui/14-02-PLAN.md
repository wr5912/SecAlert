...
- 健康状态: online(绿色), offline(红色), warning(黄色), 同步时间显示
</context>
...
<task type="auto">
  ...
  <done>
    frontend/src/types/ingestion.ts 存在，包含 DeviceType, LogFormat, ConnectionConfig, DataSourceTemplate, WizardState, DataSourceStatus 类型，包含 DEVICE_TYPES, LOG_FORMATS, WIZARD_STEPS 常量
  </done>
</task>
...
<task type="auto">
  ...
  <done>
    frontend/src/stores/ingestionStore.ts 存在，包含 useIngestionStore hook，包含所有状态和 actions
  </done>
</task>
...
<task type="auto">
  ...
  <done>
    frontend/src/api/ingestionEndpoints.ts 存在，包含 useTemplates, useCreateTemplate, useUpdateTemplate, useDeleteTemplate hooks，使用 TanStack Query
  </done>
</task>
...
<task type="auto">
  ...
  <done>
    WizardModal.tsx, StepIndicator.tsx, Step1DeviceType.tsx, Step2Connection.tsx, Step3LogFormat.tsx, Step4Complete.tsx 都存在，向导组件完整
  </done>
</task>

<task type="auto">
  <name>Task 2.5: 创建模板管理组件（含健康状态 DI-06）</name>
  <files>
    - frontend/src/components/ingestion/TemplateCard.tsx
    - frontend/src/components/ingestion/TemplateEmptyState.tsx
  </files>
  ...
  <action>
    ...
    1. **TemplateCard.tsx** - 模板列表卡片（含健康状态 DI-06）：
    ```tsx
  ...
  <done>
    TemplateCard.tsx 存在，包含健康状态显示（状态图标、状态标签、同步时间），TemplateEmptyState.tsx 存在，组件使用正确的文案
  </done>
</task>

<task type="auto">
  <name>Task 2.6: 创建 IngestionPage 页面</name>
  ...
  <action>
    创建 `frontend/src/pages/IngestionPage.tsx`，整合所有 ingestion 组件：
    ...
    export function IngestionPage() {
      ...
      const handleDeleteConfirm = async () => {
        ...
        try {
          ...
          toast.success(`模板「${deleteTarget.name}」已删除`);
    ...
    参考 SettingsPage.tsx 的页面结构和 14-UI-SPEC.md 的设计规范。
  </action>
  <verify>
    <automated>ls frontend/src/pages/IngestionPage.tsx</automated>
  </verify>
  <done>
    IngestionPage.tsx 存在，整合 TemplateCard、TemplateEmptyState、WizardModal 组件，包含模板列表和新建向导功能
  </done>
</task>

<task type="checkpoint:human-verify">
  <name>Task 2.7: 人工验证 - 向导 UI</name>
...
6. 测试步骤导航: 上一步/下一步按钮
7. 验证模板列表页面正常显示
8. 验证模板卡片显示健康状态（在线/离线/警告状态）和最后同步时间
  </how-to-verify>
  <done>
    向导 UI 和模板管理页面通过人工验证，包含健康状态显示功能
  </done>
...
- 模板列表页面可以显示所有模板
- 模板可以编辑和删除
- 模板卡片显示健康状态（online/warning/offline）
- 模板卡片显示最后同步时间
</success_criteria>
