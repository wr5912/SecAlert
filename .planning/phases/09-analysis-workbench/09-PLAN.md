...
Wave 0 创建所有组件骨架和页面 stub，为后续 Wave 提供依赖。这些任务可并行执行。
...
- 加载状态、错误状态、空状态占位
- 按置信度降序排序
- 注意: StorylineFilters 是 type，不要作为 component export
</action>
<verify>
...
<done>组件文件存在且包含默认导出（StorylineFilters 不作为 component export，只在类型定义中使用）</done>
</task>
...
<done>组件文件存在，订阅 analysisStore，AI 建议包含 reasoning</done>
</task>
...
<task>
<name>Task W0-14: 页面 Stub 文件创建</name>
<files>
  - frontend/src/pages/AlertCenterPage.tsx
  - frontend/src/pages/AttackGraphPage.tsx
  - frontend/src/pages/TimelinePage.tsx
...
<action>
创建所有页面 stub 文件作为占位符（供 Task 1-01 路由配置导入使用）：

1. `AlertCenterPage.tsx` - 导出简单组件，包含 "AlertCenter Page Stub"
2. `AttackGraphPage.tsx` - 导出简单组件，包含 "AttackGraph Page Stub"
3. `TimelinePage.tsx` - 导出简单组件，包含 "Timeline Page Stub"
4. `HuntingPage.tsx` - 导出简单组件，包含 "Hunting Page Stub"
5. `AssetContextPage.tsx` - 导出简单组件，包含 "AssetContext Page Stub"

这些 stub 文件在后续 Wave (2-6) 中会被完善。
</action>
<verify>
<automated>test -f frontend/src/pages/AlertCenterPage.tsx && test -f frontend/src/pages/AttackGraphPage.tsx && test -f frontend/src/pages/TimelinePage.tsx && test -f frontend/src/pages/HuntingPage.tsx && test -f frontend/src/pages/AssetContextPage.tsx</automated>
</verify>
<done>5 个页面 stub 文件存在，可被路由配置导入</done>
</task>
...
<task>
<name>Task 1-02: Analysis Shell + NavSidebar 完善</name>
<files>
  - frontend/src/components/analysis/AnalysisShell.tsx
  - frontend/src/components/analysis/NavSidebar.tsx
</files>
...
<done>AnalysisShell 三栏布局: 左侧NavSidebar (200px), 中央Canvas (flex-1), 右侧AIPanel (320px)。NavSidebar 4 个导航项，当前选中项有 #06b6d4 左边框指示器</done>
</task>
...
- 空状态显示 "暂无告警" + 友好提示
- 加载状态和错误状态处理
- 注意: StorylineFilters 是 type 定义，只在组件内部使用，不要作为 named export
</action>
<verify>
<automated>grep -q "confidence" frontend/src/components/analysis/StorylineList.tsx && grep -q "StorylineFilters" frontend/src/components/analysis/StorylineList.tsx</automated>
</verify>
<done>支持全部筛选维度 (D-08)，按置信度降序排序，左侧聚类面板 + 中央详情区布局。StorylineFilters 只作为类型使用，不作为 component export</done>
</task>
...
- **关键**: AI 建议必须附带 reasoning 字段，显示推理过程，例如：
  ```typescript
  ...
  ```
  reasoning 内容应包含：分析依据、关联实体、威胁等级判断逻辑
</action>
<verify>
<automated>grep -q "reasoning" frontend/src/components/analysis/AIPanel.tsx && grep -q "AISuggestion" frontend/src/components/analysis/AIPanel.tsx</automated>
</verify>
<done>自然语言查询，智能推荐，上下文感知，多格式导出 (D-10)。AI 建议包含 reasoning 字段，说明推理过程</done>
</task>
...
<done>所有类型定义完整，包含 Storyline, AttackNode, AttackEdge, TimelineEvent, StorylineFilters, AISuggestion</done>
</task>
...
| AI Copilot | 上下文感知, 智能推荐, 多格式导出, reasoning 推理过程 |
...
| M-03 | AI 建议附带"为什么"解释 | AIPanel reasoning 字段存在且显示 |
...
*计划版本: 1.3*
*创建时间: 2026-03-26*
*修订时间: 2026-03-26 (修复 Task 1-01 循环依赖: 页面 stub 移至 W0-14, Task 1-01 只做路由配置)*
*规划者: gsd-planner*
