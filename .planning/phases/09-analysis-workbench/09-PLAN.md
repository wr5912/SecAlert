...
**done:** 文件存在且包含所有必需类型定义（Storyline, AttackNode, AttackEdge, TimelineEvent, StorylineFilters）

---

### Task W0-03: Zustand 分析 Store
...
**done:** Store 文件存在且导出 useAnalysisStore hook

---

### Task W0-04: 分析 API 客户端
...
**done:** 组件文件存在，包含 StorylineCardProps 接口和默认导出

---

### Task W0-06: StorylineList 组件骨架
...
**done:** 组件文件存在，包含 ReactFlow 和 dagre 集成

---

### Task W0-08: Timeline 组件骨架
...
**done:** 组件文件存在，包含 4 个轨道配置

---

### Task W0-09: NavSidebar 组件骨架
...
**done:** 组件文件存在，包含 NavLink 导航

---

### Task W0-10: AIPanel 组件骨架
...
**done:** 组件文件存在，订阅 analysisStore

---

### Task W0-11: ContextPanel 组件骨架
...
**done:** 组件文件存在，包含 3 个标签页结构

---

### Task W0-12: HuntingWorkbench 组件骨架
...
**read_first:**
- `/home/admin/work/SecAlert/frontend/src/App.tsx` — 当前路由配置
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-CONTEXT.md` — D-01 路由驱动布局决策
...
**done:** App.tsx 包含 5 个新路由：/alerts, /graph/:storyId, /timeline, /hunting, /assets/:assetId

---

### Task 1-02: Analysis Shell 布局组件

**文件:** `frontend/src/components/analysis/AnalysisShell.tsx`

**read_first:**
- `/home/admin/work/SecAlert/frontend/src/components/layout/AppShell.tsx` — 现有布局模式
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-UI-SPEC.md` — 布局结构 (三栏: 200px左Nav, flex中央, 320px右AI)
...
**done:** 三栏布局: 左侧NavSidebar (200px), 中央Canvas (flex-1), 右侧AIPanel (320px)

---

### Task 1-03: 导航组件 NavSidebar

**文件:** `frontend/src/components/analysis/NavSidebar.tsx`

**read_first:**
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-UI-SPEC.md` — 导航项清单
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-CONTEXT.md` — D-02 混合导航折叠决策
...
**done:** 4 个导航项: 告警中心, 威胁狩猎, 溯源时间线, 资产图谱；当前选中项有 #06b6d4 左边框指示器

---

## Wave 2 — 智能告警中心 (2026-03-26)

### Task 2-01: 故事线卡片 StorylineCard

**文件:** `frontend/src/components/analysis/StorylineCard.tsx`

**read_first:**
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-FEATURE.md` — 故事线卡片要素
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-UI-SPEC.md` — 组件清单 (StorylineCard, ConfidenceScore, AlertBadge)
- `/home/admin/work/SecAlert/frontend/src/components/ui/Badge.tsx` — 现有 Badge 组件模式
...
**done:** 显示置信度评分 (红>80, 橙50-80, 灰<50)、攻击阶段标签、AI摘要、关键指标；点击导航到 /graph/:storyId

---

### Task 2-02: 故事线列表 StorylineList

**文件:** `frontend/src/components/analysis/StorylineList.tsx`

**read_first:**
- `/home/admin/work/SecAlert/frontend/src/components/AlertList.tsx` — 现有列表模式 (筛选逻辑, 加载状态)
- `/home/admin/work/SecAlert/frontend/src/stores/analysisStore.ts` — 已在 W0-03 创建
...
**done:** 支持全部筛选维度 (D-08)，按置信度降序排序，左侧聚类面板 + 中央详情区布局

---

### Task 2-03: 告警中心页面 AlertCenterPage

**文件:** `frontend/src/pages/AlertCenterPage.tsx`

**read_first:**
- `/home/admin/work/SecAlert/frontend/src/pages/AlertListPage.tsx` — 现有页面模式
...
**done:** 路由 /alerts，顶部筛选栏，左侧 StorylineList，右侧上下文面板

---

## Wave 3 — 攻击链路分析画布 (2026-03-26)

### Task 3-01: 攻击图 AttackGraph (React Flow)

**文件:** `frontend/src/components/analysis/AttackGraph.tsx`

**read_first:**
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-RESEARCH.md` — React Flow 集成模式, dagre 布局
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-FEATURE.md` — 节点类型 (主机圆形, 用户方形, IP菱形, 进程六边形)
...
**done:** React Flow 渲染，dagre 布局 (rankdir='LR')，节点拖拽/缩放，MiniMap 和 Controls

---

### Task 3-02: 攻击图页面 AttackGraphPage

**文件:** `frontend/src/pages/AttackGraphPage.tsx`

**read_first:**
- `/home/admin/work/SecAlert/frontend/src/pages/AlertDetailPage.tsx` — 现有详情页模式
...
**done:** 路由 /graph/:storyId，攻击图画布，底部范围滑块时间轴控制器 (D-06)

---

## Wave 4 — 溯源时间线 (2026-03-26)

### Task 4-01: 时间线组件 Timeline

**文件:** `frontend/src/components/analysis/Timeline.tsx`

**read_first:**
- `/home/admin/work/SecAlert/frontend/src/components/ChainTimeline.tsx` — 现有时间线参考
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-FEATURE.md` — 多轨道设计 (网络层, 终端层, 身份层, 应用层)
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-RESEARCH.md` — Pitfall 4: 虚拟化建议
...
**done:** 4 个轨道显示，展开/折叠，异常标注，虚拟化列表

---

### Task 4-02: 时间线页面 TimelinePage
...
**done:** 路由 /timeline，多轨道 Timeline，底部范围滑块时间轴控制器 (D-06)

---

## Wave 5 — 威胁狩猎工作台 (2026-03-26)

### Task 5-01: 查询构建器 QueryBuilder

**文件:** `frontend/src/components/analysis/QueryBuilder.tsx`

**read_first:**
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-RESEARCH.md` — 简化三元组查询设计
...
**done:** 三元组查询构建器，支持 AND/OR 条件，支持保存为检测规则/巡检任务

---

### Task 5-02: 狩猎工作台页面 HuntingPage

**文件:** `frontend/src/pages/HuntingPage.tsx`
...
**done:** 路由 /hunting，QueryBuilder，结果表格+可视化，历史记录

---

## Wave 6 — 资产上下文面板 (2026-03-26)

### Task 6-01: 上下文面板 ContextPanel

**文件:** `frontend/src/components/analysis/ContextPanel.tsx`

**read_first:**
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-FEATURE.md` — 资产上下文三维度
...
**done:** 3 个标签页，360度信息穿透，动态数据加载

---

### Task 6-02: 资产页面 AssetContextPage

**文件:** `frontend/src/pages/AssetContextPage.tsx`
...
**done:** 路由 /assets/:assetId，ContextPanel 展示资产信息

---

## Wave 7 — AI 调查助手 (2026-03-26)

### Task 7-01: AI Copilot 面板 AIPanel

**文件:** `frontend/src/components/analysis/AIPanel.tsx`

**read_first:**
- `/home/admin/work/SecAlert/frontend/src/stores/chatStore.ts` — 现有 AI 对话模式
- `/home/admin/work/SecAlert/.planning/phases/09-analysis-workbench/09-CONTEXT.md` — D-09 Zustand 状态管理, D-10 多格式导出
...
**done:** 自然语言查询，智能推荐，上下文感知，多格式导出 (D-10)

---

## Wave 8 — 状态管理与 API 集成 (2026-03-26)

### Task 8-01: Zustand Analysis Store

**文件:** `frontend/src/stores/analysisStore.ts`

**read_first:**
- `/home/admin/work/SecAlert/frontend/src/stores/chatStore.ts` — 现有 Zustand 模式
...
**done:** Zustand store，persist middleware，完整 Actions

---

### Task 8-02: 分析 API 端点

**file:** `frontend/src/api/analysisEndpoints.ts`

**acceptance_criteria:**
- fetchStorylines(filters): 获取告警故事线列表
- fetchAttackGraph(storylineId, timeRange): 获取攻击图数据
- fetchTimeline(timeRange, sources): 获取时间线事件
- fetchAssetContext(assetId): 获取资产上下文
- fetchHuntingResults(query): 执行威胁狩猎查询
...
**read_first:**
- `/home/admin/work/SecAlert/frontend/src/types/index.ts` — 现有类型模式
...
**done:** 所有类型定义完整，包含 Storyline, AttackNode, AttackEdge, TimelineEvent, StorylineFilters
...
*计划版本: 1.1*
*创建时间: 2026-03-26*
*修订时间: 2026-03-26 (Wave 0 明确化, 添加 automated 元素, 统一 D-06 描述)*
*规划者: gsd-planner*
