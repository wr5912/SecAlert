...
```
  ...
  <verify>
    <automated>pytest tests/test_analysis/test_classifier.py::TestChainClassifierProgram::test_rule_high_confidence_bypass -x -q 2>/dev/null || (grep -n "class AnalysisService" src/analysis/service.py && grep -n "def classify_chain" src/analysis/service.py && grep -n "def restore_chain" src/analysis/service.py)</automated>
  </verify>
...
```
  ...
  <verify>
    <automated>pytest tests/test_analysis/test_metrics.py::TestFalsePositiveMetrics::test_fp_rate_calculation -x -q 2>/dev/null || (grep -n "class FalsePositiveMetricsCollector" src/analysis/metrics.py && grep -n "def calculate_fp_rate" src/analysis/metrics.py && grep -n "fp_rate < 0.30" src/analysis/metrics.py)</automated>
  </verify>
