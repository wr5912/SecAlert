...
```
  ...
  <verify>
    <automated>pytest tests/test_analysis/test_classifier.py::TestChainClassifierProgram::test_critical_severity_override -x -q 2>/dev/null || pytest tests/test_analysis/test_classifier.py::TestChainClassifierProgram::test_suppression_threshold_low_confidence -x -q 2>/dev/null || (grep -n "class ChainClassifierProgram" src/analysis/classifier/programs.py && grep -n "def classify_with_threshold" src/analysis/classifier/programs.py && grep -n "confidence < 0.5" src/analysis/classifier/programs.py)</automated>
  </verify>
...
```
  ...
  <verify>
    <automated>grep -n "def calculate_severity" src/analysis/classifier/severity.py && grep -n "ATTACK_TECHNIQUE_SEVERITY" src/analysis/classifier/severity.py</automated>
  </verify>
...
```
  ...
  <verify>
    <automated>pytest tests/test_analysis/test_classifier.py::TestClassificationRules::test_known_false_positive_pattern -x -q 2>/dev/null || (python -c "from src.analysis.classifier.rules import ClassificationRules; r = ClassificationRules(); print(f'Loaded {len(r.get_rules())} rules'); print(r.check({'alerts': [{'alert_signature': 'COBALT_STRIKE'}]}))")</automated>
  </verify>
