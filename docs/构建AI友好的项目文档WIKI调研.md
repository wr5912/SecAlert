...
将人类友好的 `project_directory/initiative_docs`（包含会议记录、脑暴、调研报告、冗长的飞书/Slack讨论）转换为 AI 友好的 `project_directory/docs` 目录，完美契合了这一理念。
...
*   **转变**：`project_directory/docs` 必须是**预先编译好的“知识结果”**，而不是“历史记录的堆砌”。它应当像代码一样被构建，每次 `initiative_docs` 有新增，就让 AI Agent 去阅读它，并将提取的关键信息**合并（Merge）**到 `project_directory/docs` 中已有的结构化页面里。

#### 2. 以“实体与架构”为中心，打破文档边界（Entity-Centric）
人类按“文档”思考，AI 按“逻辑实体”思考。
*   **转变**：LLM 会将 `initiative_docs` 中分散在调研报告、UI设计、会议结论里的关于“用户鉴权”的信息打碎，然后在 `project_directory/docs` 中统一汇总到一个 `auth_module.md` 中。交叉引用（Cross-references）已经在文档生成时由 LLM 建立完毕。
...
*   **转变**：开发者不需要手动去写 `project_directory/docs`。你的工作是将人类资料丢进 `project_directory/initiative_docs`，然后通过 Prompt 让编码 Agent（如 Claude Code、Cursor、OpenDevin 等）自动阅读并更新 `project_directory/docs` 目录。

---

### 二、 AI友好目录的架构与组织

为了让 AI 在开发项目 project_directory 时最高效地获取上下文，`project_directory/docs` 目录的结构和元数据应该包含以下几个核心模块：

#### 1. 全局索引地图（The Master Index）—— 核心中的核心
Karpathy 强调，AI 应当**先读索引，再钻取细节（Read index first, then drill down）**。
*   **文件**：`project_directory/docs/index.md`
...
不要自己写 `project_directory/docs`。把以下两套 Prompt 存成工具，让大模型帮你做：

*   **初始化 Prompt（Init）**：
    > “读取 `project_directory/initiative_docs` 中的所有材料。你的任务是将其编译为一个结构化的技术 Wiki `project_directory/docs`。请提取业务核心逻辑、技术约束、模块设计，并生成一个分类清晰的 `index.md`，以及多个按实体划分的子 Markdown 文件。消除所有冗长的人类讨论背景，只保留结论和工程规范。”
*   **增量更新 Prompt（Update）**：
    > “我刚在 `project_directory/initiative_docs` 添加了 `0407_auth_redesign.md`。请先阅读 `project_directory/docs/index.md`，评估这个新文件影响了哪些现有的设计。请修改对应的 `docs/xxx.md` 页面以反映最新设计，并更新索引，保留原先不冲突的逻辑。不要只是追加文本，请重构融合（Synthesize）它。”
